...
window.addEventListener("resize", () => {
  ...
  if (lifts.length >= maxLifts) {
    ...
    addLiftBtn.disabled = false;
    addLiftBtn.textContent = "Add Lift";
  }
  // Adjust the layout for floors based on the new screen size
...
function adjustLayoutForMobile() {
  ...
  floors.forEach((floor) => {
    if (isMobile) {
      ...
      floor.style.flexDirection = "row";
      floor.style.alignItems = "center";
    } else {
      // Revert for desktop
      floor.style.flexDirection = "row";
      floor.style.alignItems = "flex-start";
    }
  });
}
